...
const char* Curl_strerror(struct connectdata* conn, int err)
{
    ...
    *buf = '\0';
...
#ifdef _WIN32_WCE
    {
        ...
        FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, err, LANG_NEUTRAL, wbuf,
                      sizeof(wbuf) / sizeof(wchar_t), NULL);
    ...
    else
    {
        if (!get_winsock_error(err, buf, max) &&
            !FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM, NULL, err, LANG_NEUTRAL, buf, (DWORD)max,
                            NULL))
    ...
    *outbuf = '\0';
    ...
    else
    {
        ...
        if (FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, err,
                           LANG_NEUTRAL, msgbuf, sizeof(msgbuf) - 1, NULL))
        {
            msg_formatted = TRUE;
        }
#endif
